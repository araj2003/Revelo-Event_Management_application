  ...
  return (
    <div>
      ...
      <div className="flex">
        {data.map((invite:any) => {
          return (
            <div className="flex flex-col border-4 m-5 bg-slate-200" key={invite._id}>
              <p>{invite.eventId.serverName}</p>
              <p>{invite.eventId.description}</p>
              <p>Invited By: {invite.createdBy.name}</p>
              <div className="flex flex-row">
                <button className="m-2 bg-green-500 text-white p-2 hover:bg-green-600" onClick={()=>inviteAcceptOrReject(invite._id,"accept")}>
                  Accept
                </button>
                <button className="m-2 bg-red-500 text-white hover:bg-red-600" onClick={()=>inviteAcceptOrReject(invite._id,"reject")}>
                  Reject
                </button>
              </div>
            </div>
